...
标题，输入，操作，输出，以及运行环境，真正的电脑程序同样由这些要素构成。

我们依次执行这一系列操作，材料就变成了佳肴（如果成功的话）。如果我们仔细观察这一过程：
...
例子中的 ``text = 'Hello, world!'``，就是告诉电脑我们要用 ``text`` 来代表 `'Hello, world!'` 。 ``=`` 的左边是我们指定的名字，右边是名字所要代表的值。在这行代码之后的程序中，只要出现 ``text`` 的地方，就好像 ``'Hello, world!'`` 出现在那里一样。

我们试着把 ``text`` 所代表的值改成别的文字：
...
可见只要在程序中修改 ``text`` 所代表的值，程序运行后屏幕上所显示的文字就会相应改变。就好像我们直接修改了 ``stdout.write( ... )`` 中的内容一样。
...
这里我们搞出了一个新的 *名字* ``print`` 。 ``print`` 所代表的 *值* 是一个 ``函数`` 。 ``函数`` 是从数学里借来的概念，例如：
...
像数学里的函数一样，我们给 ``print`` 不同的输入，``print`` 就会给我们相应的输出。不太一样的是，我们的 ``print`` 并非进行数值计算，而是做了其它操作。
